...
class CloakroomUI(tk.Tk):
    ...
    def __init__(self, cloakroom_instance, *args, **kwargs):
        ...
        self.tags_table_frame = TagsTableFrame(self,
        ...
        self._lbl_acquire_free_tag = tk.Label(self, bg="white")
    ...
    def _return_tag(self):
        try:
            tag = int(self._ent_tag_string.get())
            self.result = self.__cloakroom_instance.return_tag(tag)
            self._lbl_return_tag.configure(
                text=f"Возвращен номерок: {tag}"
            )
        except cr.TagAlreadyReturnedError:
            self._lbl_return_tag.configure(
                text="Номерок уже был возвращен ранее"
            )
        except ValueError:
            self._lbl_return_tag.configure(
                text=f"Неизвестный номерок."
                )
